#!/usr/bin/env python3
"""Build v4.10: Fix daily question buttons — JSON.stringify in onclick attribute
broke all question options. JSON double-quotes closed the HTML attribute early."""
...
src = open('daddy-duty-v4_9.html', encoding='utf-8').read()
...
# ── 2. Version bump ────────────────────────────────────────────────────────────
assert "v4.9 · cloud sync" in src
src = src.replace("v4.9 · cloud sync", "v4.10 · cloud sync", 1)
assert "appVersion:'v4.9'" in src
src = src.replace("appVersion:'v4.9'", "appVersion:'v4.10'", 1)
print("  - Version: v4.9 → v4.10")

# ── 3. Fix question button onclick — escape JSON for HTML attribute ─────────────
# JSON.stringify produces {"key":"value"} — the inner double-quotes closed the
# onclick="..." attribute early, leaving a truncated JS expression that threw
# SyntaxError on click. Fix: replace " with &quot; so the attribute stays intact.
# The browser decodes &quot; → " before executing onclick, so applyQuestionAnswer
# receives a proper JS object literal.
OLD_Q_RENDER = '''JSON.stringify(opt.stateUpdate)})">${opt.label}</button>`).join('')'''
NEW_Q_RENDER = '''JSON.stringify(opt.stateUpdate).replace(/"/g,'&quot;')})">${opt.label}</button>`).join('')'''
assert OLD_Q_RENDER in src, "question button onclick anchor not found"
src = src.replace(OLD_Q_RENDER, NEW_Q_RENDER, 1)
print("  - JS: renderDailyQuestion() escapes JSON with &quot; for HTML attribute safety")

# ── 4. Write output ────────────────────────────────────────────────────────────
with open('daddy-duty-v4_10.html', 'w', encoding='utf-8') as f:
    f.write(src)

os.makedirs('docs', exist_ok=True)
shutil.copy('daddy-duty-v4_10.html', 'docs/index.html')
...
# ── 5. Verify ──────────────────────────────────────────────────────────────────
content = open('daddy-duty-v4_10.html', encoding='utf-8').read()

node = shutil.which('node') or '/opt/homebrew/bin/node'
if os.path.exists(node):
    scripts = re.findall(r'<script(?!\s+src)[^>]*>(.*?)</script>', content, re.DOTALL)
    app_script = max(scripts, key=len)
    with open('/tmp/check_v4_10.js', 'w') as f:
        f.write(app_script)
    result = subprocess.run([node, '--check', '/tmp/check_v4_10.js'], capture_output=True, text=True)
    assert result.returncode == 0, f"JS syntax error:\n{result.stderr}"
    print("  - JS syntax: ✓ (node --check passed)")
else:
    print("  - JS syntax: ⚠ skipped (node not found)")

assert "v4.10 · cloud sync" in content
assert "&quot;" in content, "HTML-escaped quotes missing from question render"
assert 'JSON.stringify(opt.stateUpdate).replace' in content, "replace() call missing"
...
print("\n✓ daddy-duty-v4_10.html written")
print("  - Version: v4.10")
print("  - Fix: daily question buttons now work — JSON &quot; escaping in onclick attr")

# ── Deploy ─────────────────────────────────────────────────────────────────────
print("\nPushing to GitHub Pages...")
subprocess.run(['git', 'add', 'docs/'], check=True)
commit = subprocess.run(
    ['git', 'commit', '-m', 'deploy: v4.10 — fix daily question buttons (JSON &quot; escaping in onclick)'],
